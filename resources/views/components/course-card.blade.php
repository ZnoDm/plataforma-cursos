...
<article class="card flex flex-col">
    ...
    <div class="px-6 py-4 flex-1 flex flex-col">
        ...
        @if ($course->price->value ==0)
            <p class="my-2 text-green-500 font-bold">GRATIS</p>
        @else
            <p class="my-2 text-gray-500 font-bold">US$ {{$course->price->value}}</p>
        @endif

        <a href="{{route('courses.show',$course)}}" class="block text-center w-full mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4">Ir al Curso</a>
    </div>
</article>